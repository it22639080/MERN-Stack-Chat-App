...
const ChatContainer = () => {
  ...
  return selectedUser ? (
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
       ...
       <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
          <div className='flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
            <input onChange={(event)=> setInput(event.target.value)} value={input} onKeyDown={(e)=>e.key === "Enter" ? handleSendMessage(e): null} type="text" placeholder='send a message...' className='flex-1 text-sm p-3 border-none rounded-lg outline-none text-white placeholder-gray-400' />
            <input onChange={handleSendImage} type="file" className='hidden' id='image' accept='image/png, image/jpeg' hidden />
          <label htmlFor="image">
    ...
    <div className='flex flex-col items-center justify-center gap-2 text-gray-500'> 
      <img src={assets.logo_icon} alt="chat icon" className='max-w-16 h' />
